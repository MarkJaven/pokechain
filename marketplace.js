...
// ✅ FIXED: Only sync once and prevent duplicates
async function syncPastListings() {
  try {
    if (!window.wallet.getAccount()) return;
    
    const syncKey = `pokechain_tx_sync_${window.wallet.getAccount().toLowerCase()}`;
    const hasSynced = localStorage.getItem(syncKey);
    
    // Only sync once per wallet
    if (hasSynced) {
      console.log('✅ Past listings already synced, skipping...');
      return;
    }

    console.log('🔄 First-time sync of past listings from blockchain...');
    const provider = await window.wallet.getProvider();
    ...
    const currentUser = window.wallet.getAccount().toLowerCase();
    const filter = marketplace.filters.PokeListed(null, null, currentUser);
    const logs = await marketplace.queryFilter(filter, -10000, 'latest'); // Only last 10k blocks
    
    console.log(`📜 Found ${logs.length} past listing events`);
    
    let addedCount = 0;
    for (const log of logs) {
      const { listingId, tokenId, seller, price } = log.args;
      
      // Check for EXACT duplicate (same hash = same transaction)
      const exists = window.txHistory.notifications.some(tx => tx.hash === log.transactionHash);
      ...
      if (!exists) {
        try {
          ...
          window.txHistory.add({
            ...
            timestamp: (await log.getBlock()).timestamp * 1000,
            details: {
              pokemonName: meta?.name || `NFT #${tokenId}`,
              tokenId: tokenId.toString(),
              listingId: listingId.toString(),
              // Mark as synced to distinguish from manual listings
              isSynced: true
            }
          });
          addedCount++;
        } catch (metaErr) {
          console.warn(`⚠️ Could not load metadata for token #${tokenId}:`, metaErr);
        }
      }
    }
    
    localStorage.setItem(syncKey, 'true'); // Mark as synced
    console.log(`✅ Sync complete: ${addedCount} new listings added to history`);
  } catch (e) {
    console.error('❌ Failed to sync past listings:', e);
  }
}
